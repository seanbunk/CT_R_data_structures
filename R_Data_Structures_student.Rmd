...
my_array <- array(
  ...
  dimnames <- list(c("ROW 1", "ROW 2", "ROW 3"), c("COL 1","COL 2", "COL 3"),
                   c('MATRIX 1', "Matrix reloaded", 'Matrix tres'))
)
...
```{r}
first_v <- c(1,2,3,4,5,6,7,8,9)
second_v <- c(9,8,7,6,5,4,3,2,1)
```
...
4.  Print the second column of both matrices.
    ...
    ```{r}
    first_m <- matrix(first_v, nrow = 3, ncol = 3)
    second_m <- matrix(second_v, nrow = 3, ncol = 3)

    homework <- array(c(first_m, second_m), dim = c(3,3,2))
    homework
    ```
